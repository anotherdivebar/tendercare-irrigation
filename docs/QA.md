...
| `npm run webflow:bundle` | Passed with actual Webflow CLI 2.7.1; client and server bundles generated for 29 declarations |

Tests exercise chosen-contact validation, malformed input, independent form IDs, missing transport, confirmed and rejected delivery, retained values after errors, upload limits, mobile menu focus, package variants, server-rendered FAQ content, hidden unverified proof, empty CMS state, missing optional images, schema accuracy, canonical-origin validation, social image URLs and JSON-LD escaping.
...
The second-pass browser layout sweep covered **320, 375, 390, 430, 768, 1024, 1280 and 1440 pixels** across those twelve routes: 96 viewport/route combinations. No horizontal overflow, overflowing content element or duplicate H1 was found. Desktop, tablet and mobile screenshots were reviewed for image crops, navigation, readable copy, service directory hierarchy, package layout, FAQ controls and form usability.
...
- City/well package selection updates the visible inclusions and pressed state.
- The sticky service topic menu collapses on mobile, closes after a jump, and leaves the destination heading below the bar.
- Service/package links preselect the estimate request. Package context survives confirmed submission, disappears when the service changes, and resets for a new request. Unknown URL values are ignored.
...
3. Supply verified contact/business details, approve service/package copy and complete [CONTENT-TODO.md](CONTENT-TODO.md). Unverified reviews, contact data and unsupported claims remain omitted.
